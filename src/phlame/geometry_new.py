...
class Stack():
    ...
    Attributes:
        ...
        materials: A dictionary containing user defined materials.
        patterns: A dictionary containing user defined patterns.
    ...
    def define_material(self, material_name, permittivity, is_2d=None):
        ...
        if material_name in self.material_dict:
            ...
            self.material_dict[material_name] = Material(permittivity, is_2d)

    def define_pattern(self, pattern_name, material_name_list,
                       width_list=None):
        ...
        if pattern_name in self.pattern_dict:
            ...
            self.pattern_dict[pattern_name] = Pattern(material_list,
                                                      width_list)
...
class Material():
    ...
    Attributes:
        permittivity: 3-element numpy array of permittivities. We assume that the
            ...
            permittivity tensor.
        is_2d: A flag which is True if the material is infinitesimally
            thick. In this case, permittivity is the 2D permittivity, which has
            units of length.
        is_uniaxial: True if the material is isotropic within the plane, False
    ...
    def __init__(self, permittivity, is_2d=False):
    ...
    def set_permittivity(self, permittivity):
        ...
        Args:
            permittivity: A number or a 3-element array-like object of numbers.

        Raises:
            ValueError: Invalid format.
        """
        permittivity = np.array(permittivity).flatten()

        if len(permittivity) == 1:
            self.permittivity = [permittivity for i in range(0, 3)]
            self.is_uniaxial = True

        elif len(permittivity) == 3:
            self.permittivity = list(permittivity)
            if permittivity[0] == permittivity[1]:
                self.is_uniaxial = True
            else:
                self.is_uniaxial = False

        else:
            raise ValueError("`permittivity` must be a scalar or an array-like"
